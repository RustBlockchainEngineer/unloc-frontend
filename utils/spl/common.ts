...
export const uiAmountToAmount = (amount: number | string, mintDecimals: number): BN => {
  ...
  if (tokens.length === 1) return new BN(amount).mul(new BN(10).pow(new BN(mintDecimals)));
