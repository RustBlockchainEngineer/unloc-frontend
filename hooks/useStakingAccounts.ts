...
const fetchUserStakingAccounts = async (_: string, connection: Connection, wallet: PublicKey) => {
...
export const useStakingAccounts = () => {
  ...
  const { error, data, mutate } = useSWR<StakingAccountState[], Error>(
    publicKey ? [GET_STAKING_ACCOUNTS_KEY, connection, publicKey] : null,
    fetchUserStakingAccounts,
